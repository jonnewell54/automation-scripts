...
def setswportconfig(p_apikey, p_shardurl, p_devserial, p_portnum, p_portcfg):
    ...
    r = requests.put('https://%s/api/v1/devices/%s/switchPorts/%s' % (p_shardurl, p_devserial, p_portnum), data=json.dumps(validconfig), headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
